...
// Override COOP on pages that use Firebase signInWithPopup so the Google OAuth
// popup can communicate back to the opener. The default "same-origin" blocks
// cross-origin popup messaging; "same-origin-allow-popups" permits it while
// still protecting all other pages.
app.Use(async (ctx, next) =>
{
    if (ctx.Request.Path.StartsWithSegments("/Login") ||
        ctx.Request.Path.StartsWithSegments("/AcceptInvite"))
